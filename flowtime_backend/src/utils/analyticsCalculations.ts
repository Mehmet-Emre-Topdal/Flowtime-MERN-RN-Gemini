...
import { getDateString, todayStr } from './dateHelpers';
import { calculateStreaks } from './streakUtils';
import {
    WARMUP_RATIO,
    WARMUP_CV_THRESHOLD,
    STREAK_THRESHOLD_RATIO,
    MIN_SESSIONS_FOR_STREAK,
    MIN_SESSIONS_FOR_DAILY_WAVES,
    MIN_SESSIONS_FOR_RESISTANCE,
    MIN_SESSIONS_FOR_FLOW_WINDOW,
    MIN_SESSIONS_FOR_WARMUP,
...
export function calcDailyFlowWaves(sessions: FlowSession[]): DailyFlowWavesResult {
    ...
    if (recent.length < MIN_SESSIONS_FOR_DAILY_WAVES) {
...
export function calcResistancePoint(sessions: FlowSession[]): ResistancePointResult {
    if (sessions.length < MIN_SESSIONS_FOR_RESISTANCE) {
...
export function calcFlowStreak(sessions: FlowSession[]): FlowStreakResult {
    if (sessions.length < MIN_SESSIONS_FOR_STREAK) {
    ...
    const { recordStreak } = calculateStreaks(dailyScores, threshold);
...
export function calcTaskFlowHarmony(sessions: FlowSession[], tasks: TaskDto[]): TaskFlowHarmonyResult {
    ...
    if (taggedSessions.length < MIN_SESSIONS_FOR_RESISTANCE) {
...
export function calcWarmupPhase(sessions: FlowSession[]): WarmupPhaseResult {
    ...
    if (successfulSessions.length < MIN_SESSIONS_FOR_WARMUP) {
    ...
    if (prevMonthSessions.length >= 10) {
        ...
        prevMonthWarmup = Math.round(prevAvg * WARMUP_RATIO * 10) / 10;
